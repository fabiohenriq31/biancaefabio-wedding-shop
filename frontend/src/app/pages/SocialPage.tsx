import { Camera, Heart, Home, ImagePlus, MessageCircle, MoreHorizontal, Repeat2, Save, Send, Share, Sparkles, Trash2, User } from 'lucide-react';
      ...
      <main className="min-h-screen border-r border-[var(--wedding-beige)] bg-white">
        <header className="sticky top-20 z-10 border-b border-[var(--wedding-beige)] bg-white/90 backdrop-blur">
          ...
          <div className="grid grid-cols-2 text-center text-sm font-semibold">
            <button type="button" className="relative px-4 py-4 text-[var(--wedding-text)]">Para voce<span className="absolute bottom-0 left-1/2 h-1 w-16 -translate-x-1/2 rounded-full bg-[var(--wedding-gold)]" /></button>
        ...
        {loading ? <p className="px-5 py-8 text-[var(--wedding-text-light)]">Carregando B&F Social...</p> : posts.length === 0 ? (
          <div className="px-8 py-16 text-center text-[var(--wedding-text-light)]"><Camera className="mx-auto mb-3 h-8 w-8 text-[var(--wedding-gold)]" /><p className="text-lg text-[var(--wedding-text)]">Ainda nao tem posts por aqui.</p><p>Seja a primeira pessoa a movimentar o feed do casamento.</p></div>
        ) : posts.map((post) => {
