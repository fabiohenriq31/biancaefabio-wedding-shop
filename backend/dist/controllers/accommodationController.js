...
exports.getAdminAccommodations = getAdminAccommodations;
...
const Accommodation_1 = require("../model/Accommodation");
...
async function validateGuestIds(guestIds) {
    if (guestIds.length === 0) {
        return [];
    }
    const guests = await Guest_1.Guest.find({ _id: { $in: guestIds } });
    if (guests.length !== guestIds.length) {
        return null;
    }
    return guests;
}
function serializeAccommodation(accommodation) {
    const assignedGuests = Array.isArray(accommodation.guestIds)
        ? accommodation.guestIds.map((guest) => ({
    ...
    return {
        ...
        guestIds: assignedGuests.map((guest) => guest._id),
        assignedGuests,
        createdAt: accommodation.createdAt,
        updatedAt: accommodation.updatedAt,
    };
}
function buildSummary(accommodations) {
...
async function getAdminAccommodations(_req, res) {
    try {
        const [accommodations, guests] = await Promise.all([
            Accommodation_1.Accommodation.find()
                .populate("guestIds", "name status isChild guestType")
                .sort({ createdAt: -1 }),
            Guest_1.Guest.find().sort({ name: 1 }),
        ]);
...
async function createAccommodation(req, res) {
    try {
        ...
        const accommodation = await Accommodation_1.Accommodation.create({
            name,
...
async function updateAccommodation(req, res) {
    try {
        ...
        const accommodation = await Accommodation_1.Accommodation.findByIdAndUpdate(req.params.id, {
            name,
            type: parseType(req.body.type),
            status: parseStatus(req.body.status),
            bedDescription: sanitizeText(req.body.bedDescription, 280),
            ...
            notes: sanitizeText(req.body.notes, 800),
            guestIds,
        }, { new: true }).populate("guestIds", "name status isChild guestType");
        if (!accommodation) {
...
async function deleteAccommodation(req, res) {
    try {
        const accommodation = await Accommodation_1.Accommodation.findByIdAndDelete(req.params.id);
        if (!accommodation) {
            return res.status(404).json({ message: "Hospedagem nao encontrada." });
        }
